...
fn get_location_store() -> &'static Arc<RwLock<LocationStore>> {
    LOCATION_STORE.get_or_init(|| {
        Arc::new(RwLock::new(LocationStore::new()))
    })
}
...
impl LocationStore {
    ...
    fn get_locations_by_distance(&self, latitude: f64, longitude: f64) -> Vec<(Location, f64)> {
        ...
        locations_with_distance.sort_unstable_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
